...
- **前提**：テスト環境で、フォーム項目ID/設問メタ情報の保存が欠落する
    - formIdが保存されていない
    - quizIdとformIdが不整合
- **手順**
  1. 対応不能な状態で送信
...
- **目的**  
  Attemptの削除が物理削除ではなく論理削除として処理されることを確認する。

- **前提**  
  - 既に少なくとも1件のAttemptが存在する  
  - Attemptに対応するAttemptAnswersが存在する  
  - 教師が削除操作権限を持つ  

- **手順**  
  1. 教師が対象Attemptを指定する  
  2. 削除操作を実行する  
  3. Attemptsシートの該当レコードを確認する  

- **期待結果**  
  - 該当Attemptレコードが物理削除されない  
  - 以下の列が更新される  
    - `isDeleted = true`  
    - `deletedAt` に削除日時が記録される  
    - `deletedBy` に教師アカウントが記録される  
  - 他の列データは保持される  
...
## TS-20 論理削除連動（AttemptAnswers）

- **目的**  
  Attemptが論理削除された場合、対応するAttemptAnswersも連動して論理削除されることを確認する。

- **前提**  
  - TS-19でAttemptが論理削除済み  
  - 当該Attemptに紐付くAttemptAnswersが複数存在する  

- **手順**  
  1. Attempt削除後、AttemptAnswersシートを確認する  
  2. 該当attemptIdを持つレコードを抽出する  

- **期待結果**  
  - すべての該当AttemptAnswersレコードに対して  
    - `isDeleted = true` が設定されている  
    - `deletedAt` が記録されている  
    - `deletedBy` が記録されている  
  - 物理削除は行われていない  
...
## TS-21 論理削除データが集計に含まれない

- **目的**  
  論理削除されたAttemptおよびAttemptAnswersが集計対象から除外されることを確認する。

- **前提**  
  - 少なくとも3件のAttemptが存在する  
  - うち1件をTS-19により論理削除済み  
  - それぞれ異なる得点結果を持つ  
...
- **期待結果**  
  - 論理削除されたAttemptは平均点計算に含まれない  
  - 論理削除されたAttemptAnswersは正答率算出に含まれない  
  - 削除前と削除後で集計値が変化する  
  - 集計処理はエラーなく完了する  
